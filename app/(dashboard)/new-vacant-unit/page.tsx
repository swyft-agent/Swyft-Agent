...
export default function NewVacantUnitPage() {
  ...
  }, [user?.company_account_id, user?.id])
  ...
  const handleArrayToggle = (field: "amenities" | "utilities_included", value: string) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleImageUpload = async (files: FileList) => {
    if (!files.length || !user?.company_account_id) return
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Error creating vacant unit:", error)
      setErrors({ general: "Failed to create vacant unit. Please try again." })
    } finally {
      setLoading(false)
    }
  }
                  ...
                  {UTILITIES.map((utility) => (
                    <div key={utility} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={() => handleArrayToggle("utilities_included", utility)}
                      />
